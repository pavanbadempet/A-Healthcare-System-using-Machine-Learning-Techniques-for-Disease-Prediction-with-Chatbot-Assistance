...
class MLService:
    ...
    def predict_liver_disease(self, age, gender, total_bilirubin, alkaline_phosphotase, alamine_aminotransferase, albumin_globulin_ratio):
        if not self.liver_model or not self.scaler:
        ...
        # Input order for raw data construction: 
        # [age, gender_val, total_bilirubin, alkaline_phosphotase, alamine_aminotransferase, albumin_globulin_ratio]
        
        # BUT the logic in frontend was:
        # data[skewed] = np.log1p(data[skewed])
        # data[attributes] = scaler.transform
        
        # Ideally we replicate EXACT logic from frontend
        import pandas as pd
        input_list = [age, gender_val, total_bilirubin, alkaline_phosphotase, alamine_aminotransferase, albumin_globulin_ratio]
